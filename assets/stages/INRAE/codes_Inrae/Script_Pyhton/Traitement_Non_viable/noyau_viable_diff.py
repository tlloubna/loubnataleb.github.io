...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 10:56:20 2024
...
from mpl_toolkits.mplot3d import Axes3D  # Fonction pour les graphiques en 3D
import numpy as np

# Fonction pour extraire les paramètres d'un fichier
def extract_parameters(file_path, viable):
    try:
        if viable:
            # Charger les données des fichiers viables
            data = pd.read_csv(file_path, delim_whitespace=True, skiprows=1)
            data = data.iloc[:-1]  # Supprimer la dernière ligne
        else:
            # Charger les données des fichiers non viables
        ...
        # Récupérer et trier les années uniques
        ...
        # Calculer l'indice de Gini moyen
...
# Fonction pour charger et traiter les fichiers
...
# Définir les chemins des fichiers de sortie :Regarder ou t'a le fichier 
...
# Liste des fichiers non viables colorés en bleu
blue_files = []
...
z_viable = df_viable["gini_index"]
y_viable = df_viable['surface_terriere']
x_viable = df_viable['num_species']
z_Nviable = df_non_viable["gini_index"]
y_Nviable = df_non_viable['surface_terriere']
x_Nviable = df_non_viable['num_species']
...
# Classifier les points non viables selon leurs caractéristiques
for index, (gini, basal, wood_harvested) in enumerate(zip(x_Nviable, y_Nviable, z_Nviable)):
    ...
    elif wood_harvested < 0.25 or wood_harvested > 0.75:
        colors_non_viable.append('gray')
...
# Tracer les points pour les individus viables et non viables en 3D
ax.scatter(x_viable, y_viable, z_viable, label='Individus Viables', c="red")
ax.scatter(x_Nviable_gini, y_Nviable_gini, z_Nviable_gini, label='Non Viables (nboFspecies)', c='purple')
ax.scatter(x_Nviable_basal, y_Nviable_basal, z_Nviable_basal, label='Non Viables (Surface terrière)', c='orange')
ax.scatter(x_Nviable_other, y_Nviable_other, z_Nviable_other, label='Non Viables (Gini)', c='gray')

# Configurer les axes et le titre du graphique 3D
plt.title("Individus Viables et Non Viables n_init=1000")
ax.set_xlabel('nbofspecies')
ax.set_ylabel('Surface terrière (m2/ha)')
ax.set_zlabel('GINI', rotation=0)
...
# Affichage des variables en 2D
# Variables à afficher
variables = ['gini_index', 'surface_terriere', 'wood_harvested', 'num_species', 'num_trees', 'mortality_rate']
colors = ['red', 'blue']  # Couleurs pour les variables viables et non viables

# Création d'une figure avec des sous-graphiques en 2D
fig, axs = plt.subplots(nrows=3, ncols=2, figsize=(15, 15))
axs = axs.flatten()  # Aplatir les axes pour itération facile

for i, var in enumerate(variables):
    # Tracer les données pour les individus viables et non viables
    axs[i].scatter(df_viable.index, df_viable[var], color='red', label='Viables')
    axs[i].scatter(df_non_viable.index, df_non_viable[var], color='blue', label='Non Viables')
    axs[i].set_title(f'{var} en fonction de l\'index')
